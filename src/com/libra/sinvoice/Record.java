...
public class Record {
	...
	public Record(Callback callback, int frequence, int channel, int bits,
			int bufferSize) {
		mState = STATE_STOP;
	...
	public void start() {
		if (STATE_STOP == mState) {
			...
			int minBufferSize = AudioRecord.getMinBufferSize(mFrequence,
					mChannelConfig, mAudioEncoding);
			LogHelper.d(TAG, "minBufferSize:" + minBufferSize);

			AudioRecord record = new AudioRecord(MediaRecorder.AudioSource.MIC,
					mFrequence, mChannelConfig, mAudioEncoding, mBufferSize);
			...
			if (null != mCallback) {
				...
				while (STATE_START == mState) {
					...
					if (null != data) {
						if (null != data.mData) {
							int bufferReadResult = record.read(data.mData, 0,
									mBufferSize);
